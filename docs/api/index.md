# API Documentation

These documents define the REST API foundation, ownership, request and
response conventions, validation, security, errors, and versioning.

## Documents

- [API foundation](api-foundation.md)
- [API security boundaries](api-security-boundaries.md)
- [Correlation, pagination, and filtering](correlation-pagination-filtering.md)
- [Error taxonomy](error-taxonomy.md)
- [HTTP status and validation](http-status-and-validation.md)
- [Middleware order](middleware-order.md)
- [Module API ownership](module-api-ownership.md)
- [OpenAPI ownership](openapi-ownership.md)
- [Request and response standards](request-response-standards.md)
- [Versioning and deprecation](versioning-and-deprecation.md)
